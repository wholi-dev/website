...
export const AddressValidationForm: React.FC<AddressValidationFormProps> = ({
  ...
  const [previousSubmittedAddress, setPreviousSubmittedAddress] =
    useState<AddressObject | null>(null);
  const { validationResult, isValidating, error, validateAddress } =
  ...
  const handleFormValidation = (validationResult: any) => {
    ...
    addressComponents.forEach((component: any) => {
      if (component.confirmationLevel === "UNCONFIRMED_AND_SUSPICIOUS") {
        if (
          ...
          component.componentType === "route"
        ) {
          updatedErrorFields.street =
            "Please double-check the street address before saving.";
    ...
    missingComponentTypes.forEach((componentType: string) => {
      if (componentType === "street_number" || componentType === "route") {
        updatedErrorFields.street =
          "Address not found. Please double-check before saving.";
    ...
    const hasErrors = Object.values(updatedErrorFields).some(
      (error) => error !== "",
    );
    // make sure there is an error if the granularity is OTHER
    if (!hasErrors && validationGranularity === "OTHER") {
      updatedErrorFields.street =
        "Address not found. Please double-check everything before saving.";
      updatedErrorFields.city = "Please double-check the city.";
      updatedErrorFields.state = "Please double-check the state.";
      updatedErrorFields.zip = "Please double-check the zip code.";
      setErrorFields(updatedErrorFields);
  ...
  const handleFormSubmit = async () => {
    ...
    const updatedErrorFields = {
      street:
        address.street.trim() === "" ? "Please enter a street address." : "",
    ...
    const addressRepeated =
      JSON.stringify(address) === JSON.stringify(previousSubmittedAddress);
    // this line has to be called after. Might have to change for javascript weirdness
    setPreviousSubmittedAddress(address);
    const extractedValidatedAddress =
      extractAddressComponents(validatedAddress);
    const hasAPIDifferences = !addressesAreEqual(
      address,
      extractedValidatedAddress,
    );
  ...
  const handleSaveAddress = () => {
    ...
    const result: AddressResult = {
      ...
      validationGranularity:
        validationResult?.verdict.validationGranularity ?? "UNKNOWN",
    };
        ...
        <div>
          ...
          <div className="space-y-4">
            ...
            >
              ...
              <p>
                {address.street}
                {address.aptSuite ? " " + address.aptSuite : ""}, {address.city}
            ...
            >
              <div className="flex items-center mb-2">
                <input
                  ...
                  className="mr-2"
                />
                <label htmlFor="validatedAddress" className="font-medium">
                  Recommended Address:
                </label>
              </div>
              {validationResult ? (
                <HighlightedAddress
                  userAddress={address}
                  validatedAddress={extractAddressComponents(
                    validationResult.address,
                  )}
                />
              ) : (
                <p>
                  We're sorry. There's been an error validating the address.
                </p>
              )}
              {selectedAddress === "api" && (
                <button
        ...
        <>
          <form>
            <div className="mb-4">
              ...
              />
              {errorFields.street && (
                <p className="err-msg">{errorFields.street}</p>
              )}
            </div>
            <div className="mb-4">
            ...
            <div className="mb-4">
              ...
              />
              {errorFields.city && (
                <p className="err-msg">{errorFields.city}</p>
              )}
            </div>
            <div className="mb-4">
              ...
              />
              {errorFields.state && (
                <p className="err-msg">{errorFields.state}</p>
              )}
            </div>
            <div className="mb-6">
